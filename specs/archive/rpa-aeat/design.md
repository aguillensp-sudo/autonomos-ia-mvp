...
### Amendment (Task 4.0.11, Product Owner manual execution) — real AEAT selectors and timing corrected

**Verified against real AEAT DOM — Product Owner executed Task 4.0.11 successfully on 2026-07-23.** The selectors and timing below were written from the functional spec without access to the live site; the first real end-to-end run against `https://sede.agenciatributaria.gob.es` surfaced four mismatches, all now corrected in `aeat_m303.yml` and `autenticacion.py`:

- **`boton_acceso`**: spec assumed `a#accesoClaveMovil` (an `<a>` with that id does not exist on the real page). The real element is `<button class="... js-deskpot-info ...">Cl@ve Móvil</button>` — a `<button>`, no `id`, distinguished from a sibling info-icon button (`js-movil-info`) sharing the same `name` attribute only by its class. Corrected selector: `button.js-deskpot-info:has-text('Cl@ve')`.
- **`url_autenticada_patron`**: spec assumed `*sede.agenciatributaria.gob.es*`. The real post-authentication redirect for the IVA/Modelo 303 flow lands on a different subdomain, `https://www6.agenciatributaria.gob.es/wlpl/BUGC-JDIT/VentanaCensalIva?forigen=pre303`. Additionally, Playwright glob patterns for `wait_for_url` do not let a single `*` cross a `/` (only `**` does) — `*www6.agenciatributaria.gob.es*` therefore never matches a URL containing `https://`. Corrected pattern: `https://www6.agenciatributaria.gob.es/**`.
- **`nif_autenticado_label`**: spec assumed `span#nifAutenticado` (does not exist). The real element is `<input type="text" id="nif" value="...">` — an `<input>`, not a `<span>`, so the NIF is in the `value` attribute, not text content. Corrected selector: `input#nif`; `autenticacion.py` now reads it with `.input_value()` instead of `.text_content()` (which always returns empty for an `<input>`).
- **QR render latency**: observed ranging from ~2s to over 60s across repeated real runs (highly variable, possibly influenced by AEAT-side caching/session warm-up or anti-automation throttling — not conclusively isolated). `capturar_qr_clave()` now does an explicit `locator.wait_for(state="visible", timeout=120_000)` before the screenshot, instead of a single immediate attempt.
- **`wait_for_url` load state**: AEAT's pages carry continuous background network activity (trackers/polling) that can prevent the `"load"` event from ever firing. Both the initial `page.goto()` and the post-authentication `page.wait_for_url()` now pass `wait_until="domcontentloaded"` instead of relying on the Playwright default (`"load"`).
- **Entry point matters**: navigating to the generic AEAT home page and reaching Cl@ve Móvil via the generic "ÁREA PERSONAL" account menu authenticates successfully but AEAT closes that session itself within seconds, since it isn't tied to any specific trámite. The RPA must enter via a URL carrying a `ref` tied to the IVA procedure (`.../SelectorAccesos.html?ref=%2Fwlpl%2FBUGC-JDIT%2FVentanaCensalIva%3Fforigen%3Dpre303&aut=CPE`) for the resulting authenticated session to remain stable.
