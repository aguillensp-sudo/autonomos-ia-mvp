...
**Realtime.** `ALTER PUBLICATION supabase_realtime ADD TABLE presentacion;` (migration). The frontend subscribes to `postgres_changes` `UPDATE` events on `presentacion` **unfiltered** and re-fetches `/estado` on any event — `presentacion.id` is a separate server-generated UUID, not `proceso_id` (the LangGraph thread_id), so Realtime's single-column filter can't express the real lookup key (`user_id`+`proceso`+`ejercicio`+`periodo`); see the Task 10 amendment below for the bug this replaced. RLS applies to Realtime subscriptions the same as to queries, so cross-user leakage is structurally impossible even unfiltered — the subscription only ever delivers rows this user's RLS policies already allow.
...
## SPEC-F5-07 — Post-adversarial-review fixes (Section 15)

The first `/adversarial-review` pass found 2 CRITICAL + 2 HIGH findings, both CRITICALs sharing one root cause. Per `CLAUDE.md` §7, documented here before implementation.

### CRITICAL 1+2 — neither invoice-entry path can reach a successful `/calcular`

**Problem.** `FacturaEmitida`/`FacturaRecibida` (`src/fiscal/models.py`) require `id`, `user_id`, `numero_factura` (emitida only), `fecha`, and `cuota_iva` (emitida)/`porcentaje_deducible` (recibida) with no defaults. Neither of this phase's two invoice-entry mechanisms ever supplies them:

- **Chat path** (`src/agent/nodes/recopilar_datos.py`'s `_procesar_bloques_respuesta`): appends the LLM's `agregar_factura_emitida`/`agregar_factura_recibida` tool-call input verbatim — only `{base_imponible, tipo_iva, cuota_iva, nif_cliente}` (emitida) or `{categoria_gasto, base_imponible, tipo_iva, cuota_iva, requiere_confirmacion, nif_proveedor}` (recibida). Missing on both: `id`, `user_id`, `fecha`. Missing on recibida only: `porcentaje_deducible` (not part of the tool schema at all).
- **Structured/OCR path** (`frontend/components/facturas/FacturaReviewer.tsx`): `CAMPOS_POR_TIPO` only collects `{nif_emisor|nif_proveedor, fecha, base_imponible, tipo_iva}`. Missing on both: `id`, `user_id`, `numero_factura` (emitida)/nothing extra (recibida uses `numero_factura: str | None = None`, optional), `cuota_iva` (emitida — never computed anywhere in the component). Missing on recibida only: `categoria_gasto` is collected by OCR but never rendered/editable in this component; `porcentaje_deducible` is never collected at all. Also: the emitida field is submitted as `nif_emisor`, but `FacturaEmitida`'s actual field is `nif_cliente` — Pydantic's default `extra="ignore"` behavior means this would silently vanish even once the missing-required-fields crash is fixed.

Either path reaching `calcular()` (`src/agent/nodes/calcular.py:24-25`, `FacturaEmitida(**f)`/`FacturaRecibida(**f)`) raises an uncaught `pydantic.ValidationError` → unhandled 500, no recovery path in the UI. This was masked by every test in the suite: all 5 E2E specs use a zero-invoice "Sin actividad este trimestre" message, and `ocr-upload.spec.ts` mocks `/calcular`'s response instead of exercising the real endpoint with `FacturaReviewer`'s real output shape.

**Resolution — backend (`recopilar_datos.py`).** After `_procesar_bloques_respuesta` returns, enrich every new emitida/recibida dict before appending to `facturas_emitidas`/`facturas_recibidas`: `id=str(uuid4())`, `user_id=estado["user_id"]`, `fecha=date.today().isoformat()` (chat descriptions don't specify an exact invoice date; today is a defensible default — the invoice belongs to "this quarter", and exact day within the period is immaterial for M303 aggregation). Emitida additionally gets `numero_factura=f"CHAT-{uuid4().hex[:8]}"` (mirrors `_resolver_confirmaciones`'s existing `f"OCR-{uuid4().hex[:8]}"` convention for the OCR-confirmation path). Recibida additionally gets `porcentaje_deducible`: if the LLM's tool call didn't include one, look it up from `src/fiscal/iva/tabla_deducibilidad.py::TABLA_DEDUCIBILIDAD` by `categoria_gasto` (that module's own docstring already describes its percentages as "the DEFAULT the agent proposes" — this is the intended, canonical use, not a new parallel fiscal decision; if the category isn't found in the table, default to `Decimal("0")`, the same conservative fallback the table itself uses for its own dudoso categories).

**Resolution — frontend (`FacturaReviewer.tsx`).** `CAMPOS_POR_TIPO` gains two new recibida-only entries, `categoria_gasto` and `porcentaje_deducible` — neither has an OCR-reported `confianza_*` field, so the component's existing confidence-gate logic (`Number(extracted[...]) || 0 < 0.8`) naturally treats them as always-low-confidence, forcing the same explicit-edit-or-confirm gate already built for genuinely low-confidence OCR fields (no new UI mechanism needed). `porcentaje_deducible` defaults to `"0"` (never over-claim a deduction by default, matching the fiscal table's own convention). On `handleContinuar`, the component now computes `cuota_iva = round(base_imponible * tipo_iva) / 100` and adds `id` (`crypto.randomUUID()`), `user_id` (new required prop, threaded from `page.tsx`'s existing session state through `FacturaUploader`), and — emitida only — `numero_factura` (`` `OCR-${crypto.randomUUID().slice(0,8)}` ``) and remaps `nif_emisor` → `nif_cliente` in the submitted object (the editable UI field itself is unchanged, since that's the key OCR's own extraction schema uses for `confianza_nif_emisor`). No fiscal table is duplicated client-side — `porcentaje_deducible`'s default is a conservative constant, not a category lookup, keeping the single canonical percentage table in `src/fiscal/`.

### HIGH 3 — `calcular_graph`'s recompute branch can raise `KeyError('resultado_m303')`

**Problem.** `thread_id` is deterministic (`user_id:P04:ejercicio:periodo`). If a thread's checkpoint already reached a terminal state from an earlier full run (confirmed and completed, or corrupted by a prior crash mid-flow), `calcular_graph`'s non-idempotent branch (`src/api/graph_runtime.py:145-155`) calls `grafo.invoke(None, config)`, which can no-op without executing any further nodes — `resultado["resultado_m303"]` then raises `KeyError`, an opaque, undiagnosable error from the caller's perspective. Root-caused and reproduced in a prior verification pass; confirmed unfixed in the first adversarial review.

**Resolution.** After `grafo.invoke(None, config)`, check `resultado.get("resultado_m303") is not None` before indexing. If still `None`, raise a new `GrafoEstadoTerminalError(thread_id)` — a clear, typed exception (mirroring `PresentacionDuplicadaError`'s existing pattern) — instead of letting a `KeyError` propagate. The router (`src/api/routers/p04.py`'s `/calcular` handler) catches it and returns `409` with `error="ESTADO_TERMINAL"`, telling the client to call `/iniciar` again (which resets the checkpoint's channel values, including `resultado_m303: None`) before retrying — the same recovery action a `PresentacionDuplicadaError` implicitly requires.

### HIGH 4 — `ConfirmacionModal`'s IBAN has no format validation before a real AEAT domiciliación filing

**Problem.** `docs/frontend-standards.md` mandates Zod validation on all user input before it reaches the API; `zod` is an installed dependency used nowhere in the codebase. The only gate on the IBAN feeding `POST /confirmar`'s domiciliación payload was `iban.length < 4` (`app/proceso/p04/page.tsx`) — a 4-character non-IBAN string satisfies it.

**Resolution.** `frontend/lib/validation/iban.ts` (new, shared so both call sites use one implementation): a Zod schema validating Spanish IBAN format (`ES` + 22 digits) *and* the ISO 13616 MOD-97 checksum (not just a regex — a syntactically-shaped but checksum-invalid IBAN must also be rejected, since this feeds a real bank filing). `ConfirmacionModal.tsx` validates `iban` against this schema before calling `onConfirm()` in `handleConfirmar` — an invalid IBAN shows an inline error and never calls `onConfirm` (so `/confirmar` is never reached), matching the deliberate-click requirement's own spirit (a deliberate click on invalid data still isn't a valid confirmation). `page.tsx`'s pre-modal "Continuar a la presentación" button reuses the same schema for its own disabled-state gate, replacing the previous `length < 4` placeholder, so the user sees the problem before ever opening the modal, not just at the last step.
